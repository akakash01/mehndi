# 🌿 Mehendi Artist Portfolio Website

A premium, modern, and cinematic portfolio website for professional Mehendi Artists. Designed to showcase artistry, attract bridal clients, and provide a luxury user experience through elegant animations and clean design.

---

## ✨ Overview

This website is built to create a memorable first impression and help Mehendi Artists present their work professionally. The design combines luxury aesthetics, smooth animations, and responsive layouts to deliver a high-end portfolio experience.

### Goals

- Showcase mehendi designs professionally
- Generate booking inquiries
- Build a strong personal brand
- Create an immersive user experience
- Highlight services and portfolio work

---

## 🎯 Features

### Cinematic Hero Section
- Full-screen luxury landing page
- Scroll-based mehendi drawing animation
- Realistic hand and mehendi cone visuals
- Smooth text reveal effects
- Interactive call-to-action buttons

### About Section
- Artist introduction
- Experience and expertise
- Professional journey and story
- Elegant split-screen layout

### Services Section
- Bridal Mehendi
- Arabic Mehendi
- Traditional Mehendi
- Engagement Mehendi
- Festival Mehendi
- Party Mehendi
- Custom Mehendi Designs

### Portfolio Gallery
- High-quality image showcase
- Masonry-style layout
- Category filtering
- Lightbox image preview
- Smooth hover animations

### Testimonials
- Client reviews
- Animated testimonial cards
- Trust-building social proof

### Contact & Booking
- Inquiry form
- WhatsApp integration
- Email contact support
- Quick booking requests

---

## 🎨 Design System

### Theme
Luxury • Elegant • Modern • Bridal • Minimal

### Color Palette

Primary Colors:
- Matte Black (#0A0A0A)
- Luxury Gold (#D4AF37)

Secondary Colors:
- Beige (#F5E6D3)
- Ivory (#FFF8EE)
- Mehendi Green (#4A5D23)

### Typography

Headings:
- Playfair Display
- Cormorant Garamond

Body:
- Poppins
- Nunito

---

## ⚡ Animations

### Scroll Animations
- Smooth section transitions
- Text reveal effects
- Parallax scrolling
- Fade and scale animations

### Interactive Effects
- Hover animations
- 3D card effects
- Mouse-follow highlights
- Animated buttons

### Hero Animation Flow
1. Hand enters from left
2. Mehendi cone enters from right
3. Mehendi drawing begins
4. Design gradually completes while scrolling
5. Final bridal mehendi reveal

---

## 🛠 Tech Stack

### Frontend
- Next.js
- React.js
- Tailwind CSS

### Animation
- GSAP
- Framer Motion
- Lenis Smooth Scroll

### Icons & Assets
- React Icons
- Custom SVG Assets

## 📱 Responsive Design

Fully optimized for:

- Mobile Devices
- Tablets
- Laptops
- Desktop Screens

---

## 📈 SEO Features

- Optimized Metadata
- Open Graph Tags
- Structured Data
- Fast Loading Performance
- Mobile-Friendly Design
- Search Engine Optimization

### Target Keywords

- Bridal Mehendi Artist
- Mehendi Designer
- Arabic Mehendi Artist
- Wedding Mehendi Services
- Professional Mehendi Artist

---

## 🔥 Future Enhancements

- Online Appointment Booking
- Admin Dashboard
- Instagram Feed Integration
- Client Management System
- Multi-language Support
- AI Mehendi Preview Generator
- Payment Gateway Integration

---

## 📞 Contact

For project customization or collaboration:

- Instagram
- WhatsApp
- Email

---

## 📄 License

This project is licensed for personal and commercial portfolio use.

---

## 🌟 Vision

The goal of this website is to create a luxury digital experience that reflects the beauty, elegance, and craftsmanship of professional Mehendi artistry while helping artists grow their personal brand and attract more clients.